// ============================================
// JAVASCRIPT FUNCTIONALITY FOR 8_CEH COURSE
// ============================================
...
function getTabName(tabId) {
    const names = {
        ...
        'framework-arch': 'Framework Architecture',
        'exploit-modules': 'Exploitation Modules',
        'payload-gen': 'Msfvenom Payload Generation',
        'reverse-shells': 'Reverse Shell Handler',
        'encoding': 'Encoding & Evasion',
        'rat-fundamentals': 'RAT Fundamentals',
        'email-delivery': 'Email Payload Delivery',
        'cnc-comms': 'C&C Communication',
        'priv-escalation': 'Privilege Escalation',
        'lateral-movement': 'Lateral Movement',
        'data-exfil': 'Data Exfiltration',
        'av-evasion': 'Multi-Layer AV Evasion',
        'persistence': 'Persistence Mechanisms',
        'cleanup': 'Anti-Forensics & Cleanup',
...
function displayCourseProgress() {
    ...
    const progress = localStorage.getItem('ceh8_progress');
    ...
    if (!progress) {
        ...
        localStorage.setItem('ceh8_progress', JSON.stringify(initialProgress));
    }
}

function markSectionComplete(sectionId) {
    const progress = JSON.parse(localStorage.getItem('ceh8_progress') || '{}');
    ...
    localStorage.setItem('ceh8_progress', JSON.stringify(progress));
...
function updateProgressBar() {
    ...
    const total = 18; // Total sections for 8_CEH
...
function toggleConceptDetails(element) {
    const details = element.querySelector('.concept-details');
    if (details) {
        details.style.display = details.style.display === 'none' ? 'block' : 'none';
        element.style.borderColor = details.style.display === 'block' ? '#ff00ff' : '#00d4ff';
    }
}
...
function showGlossary(term) {
    const glossary = {
        'Metasploit': 'Penetration testing framework with 2261+ exploits and 596+ payloads',
        'Msfvenom': 'Standalone payload generator tool within Metasploit Framework',
        'Payload': 'Code executed on target system after successful exploitation',
        'Handler': 'Reverse connection listener that receives callbacks from victim systems',
        'RAT': 'Remote Access Trojan - malware providing unauthorized remote access',
        'C&C': 'Command & Control server that directs compromised systems',
        'Privilege Escalation': 'Exploiting vulnerabilities to gain higher system privileges',
        'Lateral Movement': 'Moving from one compromised system to another in a network',
        'AV Evasion': 'Techniques to bypass antivirus and endpoint protection',
        'Persistence': 'Methods to maintain access after initial compromise'
    };
...
function bookmarkSection(sectionId, sectionName) {
    let bookmarks = JSON.parse(localStorage.getItem('ceh8_bookmarks') || '[]');
    
    if (!bookmarks.find(b => b.id === sectionId)) {
        bookmarks.push({ id: sectionId, name: sectionName, timestamp: new Date().toISOString() });
        localStorage.setItem('ceh8_bookmarks', JSON.stringify(bookmarks));
...
function getBookmarks() {
    return JSON.parse(localStorage.getItem('ceh8_bookmarks') || '[]');
}
...
document.addEventListener('keydown', (e) => {
    ...
    // Ctrl+F for search (browser default, but we can enhance)
    if (e.ctrlKey && e.key === '/') {
        e.preventDefault();
